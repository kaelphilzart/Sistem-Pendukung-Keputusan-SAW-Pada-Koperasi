...
export default function page() {
  ...
  const [nilai_bobot, setNilai_bobot] = useState('');

  const router = useRouter();

  const [daftarKriteria, setdaftarKriteria] = useState([]);

  useEffect(() => {
      const fetchKriteria = async () => {
          try {
              const response = await axios.get('/api/daftar-kriteria');
  ...
  const handleTambah = async (e) => {
    ...
    try {
      await csrf();
      const response = await axios.post('/api/tambah-nilai', {
        id_kriteria,
        nilai_bobot,
      });
      if (response.status === 201) {
       toast.success('Tambah Nilai Pada kriteria berhasil');
            ...
            <div className="flex-auto px-4 lg:px-10 py-10 pt-0">
              <form onSubmit={handleTambah} method='post'>
                  ...
                  <input
                    ...
                    value={nilai_bobot} onChange={(e) => setNilai_bobot(e.target.value)} required/>
                </div>
